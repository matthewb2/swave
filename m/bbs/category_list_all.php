<?
	...
	$result=mysql_query("select name from zetyx_board_category_doc order by name");
...
{
 var check;
 select.exec.value="category_copy";
 check=confirm(select.board_name.value+"카테고리로 복사 하시겠습니까?");
 if(check==true) {document.select.submit();}
}
...
{
 var check;
 select.exec.value="category_move";
 check=confirm(select.board_name.value+"카테고리로 이동하시겠습니까?");
 if(check==true) {document.select.submit();}
}
...
?>
			<option value="<?=$num?>" <?=$select?>><?=$data[name]?></option>
<?
		$select="";
		$num++;
	}
...
<tr>
	<td background=images/m_back.gif align=center>
		<img src=images/m_text.gif border=0><a href=javascript:void(category_copy()) onfocus=blur()><img src=images/m_copy.gif border=0></a> <a href=javascript:void(category_move()) onfocus=blur()><img src=images/m_move.gif border=0></a>
	</td>
</tr>
<tr>
