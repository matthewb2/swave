 ...
 ******************************************************************************/
   ...
   if (isset($_COOKIE["code"])){
	...
	if($temp_filename) $config_dir=preg_replace('/lib.php/',"",$temp_filename);
	else $config_dir="";
	...
	/*******************************************************************************
 	 * install 페이지가 아닌 경우
 	 ******************************************************************************/
	if(!preg_match("/install/i",$PHP_SELF)&&file_exists($_zb_path."config.php")) {

 	 	//세션 처리 (세션은 3일동안 유효하게 설정)
		// 세션디렉토리가 없으면 디렉토리를 생성하고 권한을 쓰기로 변경
   ...
   // 1시간 경과 후 세션을 초기화
	 ...
	 if(session_id() == '') {
		...
		if($_zbDefaultSetup[nowconnect_enable]=="true") {
			...
			if(!$_SESSION["zb_last_connect_check"]||$_zb_now_check_intervalTime>$_zbDefaultSetup[nowconnect_refresh_time]) {
				...
				$_SESSION["zb_last_connect_check"];
	...
	function member_info() {
		...
		// 트위터 페이스북으로 로그인 했을 때 처리
		if ($_SESSION["zb_logged_no"] == 99 || $_SESSION["zb_logged_no"] == 88){
			$member[no] = $_SESSION["zb_logged_no"];
			$member[name] = $_SESSION["username"];
			$member[level] = 8;
		}
		else if($_SESSION["zb_logged_no"]) {
	...
	function head($body="",$scriptfile="") {
		...
		if(!preg_match("/member/",$PHP_SELF)) $stylefile="skin/$setup[skinname]/style.css"; else $stylefile="style.css";
...
function make_links_clickable($text){
    return preg_replace('!(((f|ht)tp(s)?://)[-a-zA-Zа-яА-Я()0-9@:%_+.~#?&;//=]+)!i', '<a style="font-size:9pt;color:#666666" href="$1">$1</a>', $text);
}
...
{
	return preg_match_all("/[\xA1-\xFE][\xA1-\xFE]/", $str,$matches);
}

function subject_cut($memo, $length)
	{
			if (preg_match_all("/[\xA1-\xFE][\xA1-\xFE]/", $memo,$matches))
			{
				//$memo = "ddd";
					// 한글
