...
<?
	 ...
	 ******************************************************/
	 ...
	 function print_bbs($skinname, $title, $id, $num=5, $textlen=30, $datetype="Y/m/d") {
		...
		$_zb_path = "/home/swave50/m/bbs/";
		$str = zReadFile("/home/swave50/m/bbs/latest_skin/".$skinname."/main.html");
		if(!$str) { 
	 ...
	 // 갤러리 이미지 뽑아오는 스킨
	 function print_gallery($skinname, $title, $id, $num=10, $xsize=30, $ysize=80, $xnum=10) {
		global $_zb_path, $_zb_url, $connect, $t_board, $admin_table, $HTTP_SESSION_VARS;
	 ...
	 function sql_issue($id, $connect){
		...
		$result = mysql_query("select * from zetyx_board_$id order by no desc limit 10", $connect) or die(mysql_error());

		while($data=mysql_fetch_array($result)) {
			
			$data[file_name1] = str_replace("data/$id/","data/$id/resized/", $data[file_name1]);
			
			//if(eregi("\.gif|\.png|\.jpg",$data[file_name1])) $filename = $_zb_url.$data[file_name1];
			//elseif(eregi("\.gif|\.png|\.jpg",$data[file_name2])) $filename = $_zb_url.$data[file_name2];
			if(eregi("\.gif|\.png|\.jpg",$data[file_name2])) $filename = $_zb_url.$data[file_name2];
			else $filename="data/$id/notready.gif";
			
			
			// 파일 있을 때
			if($filename) $imgList="<img style='display:block; z-index:10; width:100%; height:200px; padding:0px' class='webzine-img' data-src='http://swave.woobi.co.kr/bbs/$filename'>";
			  else $imgList="";
			//echo $imgList;
			$memo = strip_tags($data[subject]);
			$memo = stripslashes($memo);
			$memo =mb_substr($memo,0,100,'utf-8');
			//$memo =substr($memo,0,50);
			//echo "</td></tr></table>";
			array_push($c_temp, $memo);
			array_push($c_temp_img, $imgList);
			array_push($c_temp_no, $data[no]);
		}
		return array($c_temp, $c_temp_no, $c_temp_img);
	 }
	 
	 function print_issue($skinname, $title, $id, $num=10, $xsize=40, $ysize=150, $xnum=10) {
		global $_zb_path, $_zb_url, $connect, $t_board, $admin_table, $HTTP_SESSION_VARS;
		...
		list($icar, $icar_no, $icar_img) = sql_issue("icar", $connect);
		
		list($battery, $battery_no, $battery_img) = sql_issue("battery", $connect);
				
		list($forth, $forth_no, $forth_img) = sql_issue("forth", $connect);
				
		for ($i=0; $i<=4; $i++){
		echo "<div class='swiper-slide'>";	
		
			echo "<table border='0' width='95%'>";
			echo "<tr><td><span style='font-weight:bold'>전기차</span></td></tr>";
			echo "<tr><td style='margin:0px; padding: 0 0px 0 0px'><a href='view.php?id=icar&no=".$icar_no[$i]."'>".$icar_img[$i]."</a></td></tr>";
			echo "<tr><td align='center'>";
			echo "<div style='width:90%;text-align:left;'><a style='font-size:11pt;color:#000000' href='view.php?id=icar&no=".$icar_no[$i]."'>".subject_cut($icar[$i],60)."</a>";
			echo "</td></tr>";
			echo "<tr><td width='90%' style='height:10px; border-bottom:1px solid #ddd'></td></tr>";
			echo "<tr><td height='50px' style='vertical-align:middle'><span style='font-weight:bold'>4차산업</span></td></tr>";
			echo "<tr><td style='margin:0px; padding: 0 0px 0 0px'><a href='view.php?id=forth&no=".$forth_no[$i]."'>".$forth_img[$i]."</td></tr>";
			echo "<tr><td align='center'>";
			echo "<div style='width:90%;text-align:left;'><a style='font-size:11pt;color:#000000' href='view.php?id=forth&no=".$forth_no[$i]."'>".subject_cut($forth[$i],60)."</a></div>";
			echo "</td></tr>";
			echo "<tr><td width='90%' style='height:10px; border-bottom:1px solid #ddd'></td></tr>";
			echo "<tr><td height='50px' style='vertical-align:middle'><span style='font-weight:bold'>2차전지</span></td></tr>";
			echo "<tr><td style='margin:0px; padding: 0 0px 0 0px'><a href='view.php?id=battery&no=".$battery_no[$i]."'>".$battery_img[$i]."</td></tr>";
			echo "<tr><td align='center'>";
			echo "<div style='width:90%;text-align:left;'><a style='text-align:left;font-size:11pt;color:#000000' href='view.php?id=forth&no=".$battery_no[$i]."'>".subject_cut($battery[$i],60)."</a></div>";
			echo "</td></tr>";
	  ...
	  // 뉴스 미리보기
	 function print_news($skinname, $title, $id, $num=10, $xsize=200, $ysize=150, $xnum=10) {
		global $_zb_path, $_zb_url, $connect, $t_board, $admin_table, $HTTP_SESSION_VARS;
		...
		$news_category = array("IT", "신소재","우주","의료","환경");
		for ($index =4; $index <= 9; $index++){
			if ($index != 5){
				$result = mysql_query("select * from $t_board"."_$id where category=$index order by no desc limit 15", $connect) or die(mysql_error());
				...
				echo "<div class='swiper-slide' data-name='".$news_category[$index-4]."'>";
				
				while($data=mysql_fetch_array($result)) {
					
					$memo = strip_tags($data[subject]);
					$memo = stripslashes($data[subject]);

					$memo = subject_cut($memo, 30);
				
					//echo "</td></tr>";
					array_push($news_s, $memo);
					array_push($news_d, $data[no]);
					$data[file_name1] = str_replace("data/news","data/news/resized",$data[file_name1]);
					array_push($news_f, $data[file_name1]);
					array_push($news_link, $data[sitelink1]);
				...
				for ($j=0; $j<=4; $j++){
					//echo $news_s[$j];
					?>
					<div class='tap-main-td' onclick="window.location='view.php?id=news&no=<?=$news_d[$j]?>'" style='overflow:hidden; font-size:11pt; color:#666666; height:40px; border-bottom:1px solid #e1e1e1'>
						<a style='position: relative; padding:9px 0px 0px 5px; color:#000000; font-size:14px; display:inline-block' href='<?=$news_link[$j]?>'><?=$news_s[$j]?></a>
					</div>
					<?
				}
				...
				<div style="margin-top:5px; padding:5px; border-bottom:1px solid #ddd">
					<table width="100%" style="border-collapse:collapse;" cellspacing="10px">
						<tr>
						<td style="padding-right:2px;font-size:14px">
										<a style='color:#000000' href='<?=$news_link[5]?>'>
										<img style='width:100%; height:110px;' class='webzine-img' data-src='http://swave.woobi.co.kr/bbs/<?=$news_f[5]?>' />
											
									<p style="line-height:120%; padding:5px; font-size:14px"><?=subject_cut($news_s[5],27);?></p></a>
								
						</td>
										
						<td style="padding-left:2px;">
									<a style='color:#000000' href='<?=$news_link[6]?>'>
									<img style='width:100%; height:110px;' class='webzine-img' data-src='http://swave.woobi.co.kr/bbs/<?=$news_f[6]?>' />
									<p style="line-height:120%; padding:5px; font-size:14px"><?=subject_cut($news_s[6],27);?>
									</a>
						</td>
						</tr>
					</table>
				...
				for ($j=7; $j<=10; $j++){
					//echo $news_s[$j];
					?>
					<div onclick="window.location='view.php?id=news&no=<?=$news_d[$j]?>'" style='overflow:hidden; font-size:11pt; color:#666666; height:40px; border-bottom:1px solid #e1e1e1'>
						<a style='position: relative; padding-left:5px; font-size:14px; color:#000000; display:inline-block;padding-top:6px' href='<?=$news_link[$j]?>'><?=$news_s[$j]?></a>
					</div>
					<?
				}
				?>
				...
				<table id='more-<?=$index?>'>
					...
					<input class="more" style="margin:0px;padding:0px;height:40px" name="more" type="button" onclick="addnews(<?=$index?>);" value="더 많은 기사" />
