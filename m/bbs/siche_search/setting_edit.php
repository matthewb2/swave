<?
	...
	$query = mysql_query("select name,grant_view from zetyx_admin_table",$connect);
	$searchname_check = mysql_num_rows($query);

	if($searchname_check == 0){
	$siche_table_name = "siche search v0.3";
	$siche_view_level = "siche search v0.3";
	} else {
	while($row = mysql_fetch_array($query)){
	$table_name	= $row[name];
	$view_level	= $row[grant_view];
	$siche_table_name .= "$table_name";
	$siche_table_name .= "/";
	$siche_view_level .= "$view_level";
	$siche_view_level .= "/";
	}
	}

	$searchname		= $siche_table_name;
	$bbstitle			= $siche_table_name;
	$view_level		= $siche_view_level;
	$list_view_level	= "10";
	...
	$searchpoint1 = "<font color=red>";
	$searchpoint2 = "</font>";

	$pagenumpoint1 = "<font color=red>";
	$pagenumpoint2 = "</font>";

	$h_comment = "<div align=center>";
	$f_comment = "</div>";
...
<input type="hidden" name="exec" value="setting_edit">
<input type="hidden" name="exec2" value="setting_edit_ok">
...
<tr>
	<td align="right" colspan="2"><input type="image" src="siche_search/images/modify_ok.gif" onsubmit="level_check()">&nbsp;&nbsp;<img src="siche_search/images/modify_no.gif" OnClick="eval(history.go(-1));" border="0" style="cursor:hand"></td>
</tr>
</form>
</table>