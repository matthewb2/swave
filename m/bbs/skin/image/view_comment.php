<?php
	// $comment_name = str_replace('<img ','<img style="border:0px; filter:chroma(color=white);" ',$comment_name);
	// $comment_name = preg_replace("/>([^<])/","><font style=\"font: 9pt 굴림; color: #555;\">\\1",$comment_name);
	// $comment_name = preg_replace("/([^>])<\//","\\1</font></",$comment_name);
	...
	$comment_name = str_replace($key, $value, $comment_name);
	$comment_name = str_replace("노무현", "***", $comment_name);
	$comment_name = str_replace("무현", "**", $comment_name);
	$comment_name = str_replace("노알라", "***", $comment_name);
	$comment_name = str_replace("그란도", "***", $comment_name);
	$comment_name = str_replace("강간", "**", $comment_name);
	$comment_name = str_replace("로린", "**", $comment_name);
	$comment_name = str_replace("엠창", "**", $comment_name);
	$comment_name = str_replace("두부", "**", $comment_name);
	$comment_name = str_replace("nori", "**", $comment_name);
	$comment_name = str_replace("대중핵", "***", $comment_name);
	$comment_name = str_replace("핵대중", "***", $comment_name);
	$comment_name = str_replace("폭동", "**", $comment_name);
	$comment_name = str_replace("518", "**", $comment_name);
	$comment_name = str_replace("일팔", "**", $comment_name);
	$c_memo = str_replace($key, $value, $c_memo);
	}
	*/
...
<div id="IAMCOMMENT_<?=$c_data[no]?>" align="right" style="width:100%;">
	<div id="comment_<?=$c_data[no]?>">
		<table width="100%" cellSpacing="2" cellpadding="0" style="border: 1px solid #EDE8D9;margin-bottom: 3px;">
		<colgroup><col width="300px"><col><col width="140px"></colgroup>
		<tr style="height: 29px;">
			<td align="left" style="font: 9pt 굴림; padding: 5px 0 0 3px;" valign="top"><?=$c_face_image?> <?=$comment_name?>&nbsp;<?=$c_reg_date?></td>
			<td></td>			
			<td align="right">
		...
		<tr>
			<td colspan="3" valign="top" style="font: 9pt 굴림; padding:5px; line-height:1.7em">
