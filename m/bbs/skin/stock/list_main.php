...
$change = round($data[subject]-$sitelink1,2);


if ($sitelink1 == 0){	
// $rate = $change /$sitelink1;
} else {
	$rate = round($change/$sitelink1*100,2);
}


if ($change > 0 ){
	$change = "<span style='font-size:9pt; color:red'>".$change."%</span>";
	
} else $change = "<span style='font-size:9pt;color:blue'>".$change."%</span>";


if ($rate > 0 ){
	$rate = "<span style='font-size:9pt;color:red'>".$rate."%</span>";
	
} else $rate = "<span style='font-size:9pt;color:blue'>".$rate."%</span>";
...
<tr>
	...
	line-height:150%; border-bottom:0px solid #e1e1e1; 
	height:60px; padding;0px; font-size:9pt; 
	color:#000000" onclick="window.location='view.php?id=<?=$id?>&no=<?=$data[no]?>'">
		&nbsp;<img src="<?=$dir?>/images/icon_sm_page_white.gif" />&nbsp;
		<?=$reg_date?>&nbsp;&nbsp;&nbsp;
	</td>
	<td style="vertical-align:middle; line-height:150%; 
	
	border-bottom:0px solid #e1e1e1; height:60px; padding;0px; 
	font-size:10pt; color:#000000" onclick="window.location='view.php?id=<?=$id?>&no=<?=$data[no]?>'">
...
<td style="vertical-align:middle; line-height:150%; border-bottom:1px solid #e1e1e1; height:60px; padding;0px; 
font-size:10pt; color:#000000" onclick="window.location='view.php?id=<?=$id?>&no=<?=$data[no]?>'">
