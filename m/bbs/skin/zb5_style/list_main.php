...
if(preg_match('/\.(jpg|png|jpeg)$/', $data[file_name1])) {
...
if(preg_match('/\.(jpg|png|jpeg)$/', $data[file_name2])) {
	if($data[is_secret]&&!$is_admin&&$data[ismember]!=$member[no]&&$member[level]>$setup[grant_view_secret]) {
...
<tr>
	<td class="tap-main-td" onclick="window.location='view.php?id=<?=$id?>&no=<?=$data[no]?>'">
		...
		<br><font size="2" color="#666666">&nbsp;&nbsp;&nbsp;&nbsp;조회: <?=$hit?>/ 추천: <?=$vote?>&nbsp;
<?=$name?></font>
	</td>
</tr>

<script>
localStorage.setItem('oldurl','zboard.php?id=<?=$id?>');
</script>
