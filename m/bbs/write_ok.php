...
// 각종 변수 검사;;
	...
	//보안코드 검사
	if ($id == "doc") {
		if(!$member[no] || $member[level]>=9) {
			// echo "<script>alert('".$code_check."');</script>";
			if($_POST[captcha] != $_COOKIE['code']) Error("보안문자가 잘못되었습니다");
		}
	}
	...
	if ($id == "video"){
		// $sitelink1 
		if (preg_match('/&/', $sitelink1)){
			$temp = explode("&", $sitelink1);
			$sitelink1 = $temp[0];
	...
	//if(isblank($memo) || isblank($filename1)) Error("내용을 입력하셔야 합니다");
 ...
 **************************************************************************/
	...
	if($file1_size>0&&$setup[use_pds]&&$file1) {
		...
		if($file1_size>0) {
			...
			$path = "/home/swave50/www/bbs/";
			// 디렉토리를 검사함
			if(!is_dir($path."data/".$id)) { 
				@mkdir($path."data/".$id,0777);
				@chmod($path."data/".$id,0706);
			}

			
			if(file_exists($path."data/$id/".$s_file_name1)) {  // 중복파일이 있을때;
 				@mkdir($path."data/$id/".$reg_date,0777);
				if(!move_uploaded_file($file1,$path."data/$id/".$reg_date."/".$s_file_name1))
 //Error("파일업로드가 제대로 되지 않았습니다");
				$file_name1=$path."data/$id/".$reg_date."/".$s_file_name1;
				@chmod($file_name1,0706);
				@chmod($path."data/$id/".$reg_date,0706);
				
			} else {  //중복파일이 없을 때
				if(!move_uploaded_file($file1,$path."data/$id/".$s_file_name1)) Error("파일업로드가 제대로 되지 않았습니다");
				$file_name1=$path."data/$id/".$s_file_name1;   
				@chmod($file_name1,0706);
	...
	if($file2_size>0&&$setup[use_pds]&&$file2) {
		...
		if($file2_size>0) {
			...
			*/
			...
			if(!is_dir("data/".$id)) {
				mkdir("data/".$id,0777);
				@chmod("data/".$id,0777);   // 소유권을 777로?
			}

			// 중복파일이 있을때;; 
			if(file_exists("data/$id/".$s_file_name2) && $id != "webzine") {
 ...
 **************************************************************************/
	if($mode=="modify"&&$no) {
		...
		//파일을 리사이징함
			if ($id=="image"){
			//resize_image('crop',"data/image/".$s_file_name1, "data/image/resized_".$s_file_name1, 300, 300);
			//resize_image('crop',$file_name1, "data/$id/resized_".$file_name1, 100, 80);
			}
